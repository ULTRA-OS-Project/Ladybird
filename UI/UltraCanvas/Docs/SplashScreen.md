...
[`CHANGELOG.md`](CHANGELOG.md) in this directory. The port's own CMake
([`UI/UltraCanvas/CMakeLists.txt`](../CMakeLists.txt)) parses that line into
`LADYBIRD_VERSION` and `LADYBIRD_VERSION_DATE` and passes them to the compiler —
this parsing lives with the port, not in the `UltraCanvasSources` submodule, so
the port's version is not a Ladybird-specific entry in the framework repo:
...
set(LADYBIRD_VERSION      "${CMAKE_MATCH_2}")   # e.g. 0.1.0
set(LADYBIRD_VERSION_DATE "${CMAKE_MATCH_1}")   # e.g. 2026-08-28
...
copy that will drift, which is the whole reason the changelog is the single
source of truth.
...
At runtime both are loaded from under the port's `GetResourcesDir()` at
`media/appicon/Ladybird.png` and `media/images/UltraCanvas-logo.png`. The port's
CMake stages them there at build time (a POST_BUILD copy alongside the
executable); their *source* locations differ because only one of them is
Ladybird's:

| File (source) | Runtime path | What it is |
|---|---|---|
| `res/appicon/Ladybird.png` | `media/appicon/Ladybird.png` | The Ladybird mark: the black line-art knot on the white-to-lavender-to-blue disc, transparent outside the disc, so it also serves as the window and taskbar icon. Port-owned — it lives in the port's `res/`, not in the `UltraCanvasSources` submodule. |
| `UltraCanvasSources/media/images/UltraCanvas-logo.png` | `media/images/UltraCanvas-logo.png` | The UltraCanvas hexagon. The framework's own asset, shared with the rest of the toolkit — do not take a second copy into the port. |

The Ladybird mark is generated, not hand-drawn: the framework's
`scripts/make_ladybird_icon.py` renders it (the geometry, the stroke weight and
the gradient stops all in one place if the mark ever needs to be re-cut at
another size), and the rendered PNG is committed to the port at
`res/appicon/Ladybird.png`.

```sh
python3 scripts/make_ladybird_icon.py res/appicon/Ladybird.png
```
